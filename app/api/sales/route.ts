...
export async function GET() {
  const snap = await db.collection("sales").orderBy("date", "desc").get();
  return ok(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
}

export async function POST(req: Request) {
  const body = await req.json().catch(() => ({}));
  const { customerId, date, lines } = body || {};
  ...
  const total = lines.reduce(
    (s: number, l: any) => s + Number(l.qty) * Number(l.unitPrice) + Number(l.tax || 0) - Number(l.discount || 0),
    0
  );
